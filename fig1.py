...
Traces figure 1. "experiments.py"  and "data_fig1.py" must be run before this 
...
# Import quantiles data
f = open(directory+'/Data_quantiles/quantiles_m'+str(monkey+1)+'d'+str(orientation)+pop, 'rb')
quantiles = pickle.load(f)
f.close()
f = open(directory+'/Data_quantiles/quantiles_m'+str(monkey+1)+'d'+str(orientation)+pop+'_shuffle', 'rb')
quantiles_shuffle = pickle.load(f)
f.close()
...
ax.imshow(spikes[:,0,:].transpose(), 'Greys', aspect=6) 
ax.set_xticks([])
ax.set_yticks([])
ax = fig.add_axes([0.06,0.47,.25,.4])
ax.imshow(spikes[:,1,:].transpose(), 'Greys', aspect=6) 
ax.set_xticks([])
ax.set_yticks([])
ax = fig.add_axes([0.05,0.44,.25,.4])
ax.imshow(spikes[:,2,:].transpose(), 'Greys', aspect=6) 
ax.set_xticks([])
ax.set_yticks([])
...
ax.fill_between(Time, quantiles[5,:,0], quantiles[5,:,1], color=S_color, alpha=0.5)
ax.fill_between(Time, quantiles_shuffle[5,:,0], quantiles_shuffle[5,:,1], color=S_color)
ymin, ymax = ax.get_yaxis().get_view_interval()
xmin, xmax = ax.get_xaxis().get_view_interval()
...
ax.fill_between(Time, quantiles[6,:,0]*100, quantiles[6,:,1]*100, color=S_ratio_color, alpha=0.5)
ax.fill_between(Time, quantiles_shuffle[6,:,0]*100, quantiles_shuffle[6,:,1]*100, color=S_ratio_color)
ymin, ymax = ax.get_yaxis().get_view_interval()
xmin, xmax = ax.get_xaxis().get_view_interval()
ax.add_artist(pyplot.Line2D((xmin, xmin), (ymin, ymax), color='black', linewidth=1))
ax.set_xticks([])
...
ax.fill_between(Time, quantiles[3,:,0], quantiles[3,:,1], color=psi_color, alpha=0.5)
ax.fill_between(Time, quantiles_shuffle[3,:,0], quantiles_shuffle[3,:,1], color=psi_color)
...
ax.fill_between(Time, quantiles[7,:,0], quantiles[7,:,1], color=C_color, alpha=0.5)
ax.fill_between(Time, quantiles_shuffle[7,:,0], quantiles_shuffle[7,:,1], color=C_color)
